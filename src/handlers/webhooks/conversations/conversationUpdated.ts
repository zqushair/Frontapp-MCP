  ...
  protected async process(payload: WebhookPayload, server: Server): Promise<void> {
    ...
    try {
      ...
      if (conversation.assignee) {
        console.log(`[Webhook] Assigned to: ${conversation.assignee.first_name} ${conversation.assignee.last_name}`);
      } else {
      ...
      if (conversation.tags && conversation.tags.length > 0) {
        console.log(`[Webhook] Tags: ${conversation.tags.map((tag: { name: string }) => tag.name).join(', ')}`);
      } else {
