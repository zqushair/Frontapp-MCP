  ...
  protected async process(payload: WebhookPayload, server: Server): Promise<void> {
    ...
    try {
      ...
      if (message) {
        ...
        console.log(`[Webhook] Author: ${message.author.first_name || ''} ${message.author.last_name || ''}`);
        ...
        console.error(`[Webhook] Message with ID ${messageId} not found in conversation ${conversationId}`);
      }
    } catch (error: any) {
